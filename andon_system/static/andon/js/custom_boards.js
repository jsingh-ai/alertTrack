...
const headerLoadBoardBtn = document.getElementById("headerLoadBoardBtn");
const loadBoardModalEl = document.getElementById("loadBoardModal");
const loadBoardModalList = document.getElementById("loadBoardModalList");
...
const loadBoardModal = loadBoardModalEl && window.bootstrap ? new bootstrap.Modal(loadBoardModalEl) : null;
...
function renderCurrentView() {
  ...
  createBoardBtn.classList.toggle("d-none", isPreview || editorMode === "edit_existing");
  ...
  headerLoadBoardBtn?.classList.toggle("d-none", !(isPreview && state.boards.length > 1));
...
function renderLoadBoardModalList() {
  if (!loadBoardModalList) return;
  const boards = [...state.boards];
  if (boards.length <= 1) {
    loadBoardModalList.innerHTML = '<div class="text-secondary small">No additional boards to load.</div>';
    return;
  }
  const activeId = Number(state.activeBoardId);
  loadBoardModalList.innerHTML = boards
    .filter((board) => Number(board.id) !== activeId)
    .map((board) => `
      <button class="btn btn-outline-light text-start" type="button" data-load-board-id="${board.id}">
        <div class="fw-semibold">${escapeHtml(board.name)}</div>
      </button>`)
    .join("");
}
...
function wireEvents() {
  ...
  headerLoadBoardBtn?.addEventListener("click", () => {
    renderLoadBoardModalList();
    loadBoardModal?.show();
  });
  loadBoardModalList?.addEventListener("click", (event) => {
    const button = event.target.closest("[data-load-board-id]");
    if (!button) return;
    const boardId = button.dataset.loadBoardId;
    loadBoardModal?.hide();
    runSafely(() => activateBoard(boardId), "Unable to load board");
  });
